    ...
    public partial class frmAddTerm : Form
    {
        ...
        private async Task LoadServices()
        {
            ...
            if (cmbSalon.SelectedIndex != -1)
            {
                ...
                if (result != null)
                {
                    List<AppointIT.Model.Models.Service> services = result.Select(x => x.Service).ToList();
        ...
        private async void btnAddTerm_Click(object sender, EventArgs e)
        {
            try
            {
                if (ValidateChildren())
                {
                    ...
                    var selectedServiceDuration = cmbService.SelectedItem as AppointIT.Model.Models.Service;
                    ...
                    {
                        await _termService.Insert<Term>(request);
